...
export default function ProductsBytype(): JSX.Element {
  ...
  const { page, gender } = useParams();
  const navigate = useNavigate();

  React.useEffect(() => {
    if (page === '0') navigate(`/gender/${gender}/1`);
  }, [page, gender, navigate]);
    ...
    const getProduct = async () => {
      try {
        const response = await apiUrl(
          `/productos/genero/${gender}/?page=${page}`
        );
